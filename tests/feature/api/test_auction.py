...
def test_update_auction_endpoint(test_app):
    ...
    response = test_app.put("/api/v1/auctions/61077c78-e5ff-11ed-8cac-c2d0bec86bc4?type=plain", json=update_data)

    assert response.status_code == status.HTTP_200_OK
    assert "response" in response.json()

def test_get_keys(test_app):
    response = test_app.get("/api/v1/auctions/keys/61077c78-e5ff-11ed-8cac-c2d0bec86bc4")
